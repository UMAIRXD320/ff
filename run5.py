...
def log_token():
    ...
    try:
        ge = requests.get('https://graph.facebook.com/me?access_token='+tokenku[0])
...
def menu():
    ...
    try:
        ...
        ge = requests.get('https://graph.facebook.com/me?access_token='+panda)
...
def publik():
    ...
    try:
        r= requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(5000)&access_token='+tokenku[0]).json()
        z=json.loads(r.text)
        for i in z['friends']['data']:
            uid = i["id"]
            nama = i["name"]
